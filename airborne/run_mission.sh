...
BAUD="${BAUD:-115200}"
RECORD_ROOT="${RECORD_ROOT:-${XDG_DATA_HOME:-$PROJECT_DIR}/uav-mission-recordings}"
VEHICLE_PT_MODEL="${VEHICLE_PT_MODEL:-$PROJECT_DIR/models/best.pt}"
VEHICLE_ENGINE_MODEL="${VEHICLE_ENGINE_MODEL:-$PROJECT_DIR/models/best.engine}"
...
cleanup() {
  ...
  if [[ "$CAMERA_SOURCE" == "rtsp" && -f "$SCRIPT_DIR/start_g3p_viewfinder.py" ]]; then
...
args=(
  "$PYTHON" "$SCRIPT_DIR/red_white_target_server.py"
  --host "$HOST"
  --port "$PORT"
...
if [[ "$CAMERA_SOURCE" == "rtsp" && "$G3P_SETUP_NETWORK" == "1" ]]; then
  ...
  if [[ "$REQUIRE_1080P" == "1" ]]; then
    ...
    if [[ -z "$probe" && "$G3P_START_VIEWFINDER" == "1" && -f "$SCRIPT_DIR/start_g3p_viewfinder.py" ]]; then
...
"$PYTHON" -m py_compile "$SCRIPT_DIR/red_white_target_server.py"
